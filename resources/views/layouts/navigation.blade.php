<header class="flex items-center justify-between px-6 py-4 bg-white border-b-4 border-indigo-600">
    <div class="flex items-center">
        <a href="{{ route('dashboard') }}" @click="sidebarOpen = true" class="text-gray-500 focus:outline-none lg:hidden">
    ...
    <div class="flex items-center">
        ...
        <div x-data="{ dropdownOpen: false }"  class="relative">
            ...
            <div x-cloak x-show="dropdownOpen" class="absolute right-0 z-10 w-48 mt-2 overflow-hidden bg-white rounded-md shadow-xl">
                <span class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">{{ Auth::user()->name }}</span>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <a href="{{ route('logout') }}"
                        ...
                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">{{ __('Log Out') }}</a>
                </form>
            </div>
        </div>
    </div>
